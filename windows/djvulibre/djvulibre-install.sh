...
case "$platform" in
  ...
  Win32|x64) ;;
  *)
    echo "Usage: $0 [Win32|x64|x86] [Release|Debug]"
    exit 2
    ;;
esac
case "$config" in
...
dwdir=$djdir/windows/build/$config/$platform
...
run cp $djdir/windows/djvulibre/djvulibre.nsi $target
run cp $djdir/windows/djvulibre/djvulibre*.nsh $target
