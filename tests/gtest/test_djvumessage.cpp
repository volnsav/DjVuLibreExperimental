...
TEST(DjVuMessageTest, CApiLookupFunctionsHandleSmallAndLargeBuffers)
{
  ...
  ::DjVuMessageLookUpUTF8(utf8_small, sizeof(utf8_small), "plain text");
  EXPECT_STREQ("", utf8_small);

  char utf8_large[32] = {0};
  ::DjVuMessageLookUpUTF8(utf8_large, sizeof(utf8_large), "plain text");
  EXPECT_STREQ("plain text", utf8_large);

  char native_small[4] = {'x', 'x', 'x', 0};
  ::DjVuMessageLookUpNative(native_small, sizeof(native_small), "plain text");
  EXPECT_STREQ("", native_small);

  char native_large[32] = {0};
  ::DjVuMessageLookUpNative(native_large, sizeof(native_large), "plain text");
